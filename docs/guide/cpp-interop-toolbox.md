...
shader-uniform shims. The last section covers what's still blocked.
...
- **Static staging buffer (first approach).** A file-level `static float
  ...
  all of them must stay in one fn (the buffer is shared mutable state, hence the
...
`cpp/raw` block is ordinary C, and a sibling wrapper in the same block
...
process, so the smoke log comes back empty and the port looks broken when
...
**jank has native pointer interop, so the pointer-taking raylib APIs were never
...
The wider jank cpp-interop toolbox, with the
[jank book's cpp-interop chapter](https://github.com/jank-lang/jank/tree/main/book/src/cpp-interop)
as the authoritative reference on each of these:

- `(cpp/& x)` is address-of (unary); `(cpp/* p)` is dereference (unary).
...
- **APIs taking a `Vector2 *points` array** (`DrawSplineLinear`,
  ...
  two `DrawTriangle`s from the angle formulas directly, and the wrap-around falls
